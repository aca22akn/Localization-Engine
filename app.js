...
function downloadReport() {
  ...
  const lines = [];
  lines.push('UK MARKET LOCALIZATION ENGINE');
  lines.push(bar);
  lines.push(`Brand:     ${d.brand_name || '—'}`);
  if (d.website_url)      lines.push(`Website:   ${d.website_url}`);
  if (d.instagram_handle) lines.push(`Instagram: @${d.instagram_handle}`);
  lines.push(`Generated: ${date}`);
  lines.push(bar);
  lines.push('');
  lines.push(`OVERALL SCORE: ${(d.overall_score ?? 0).toFixed(1)} / 10`);
  lines.push('');
  lines.push('VERDICT');
  lines.push(thin);
  lines.push(stripHtml(d.verdict || ''));
  lines.push('');

  if (d.pakistaniPhoneNumberFound) {
    lines.push('⚠  PAKISTANI PHONE NUMBER DETECTED — significant trust risk for UK customers');
    lines.push('');
  }

  (d.dimensions || []).forEach((dim, i) => {
    const meta = DIMENSIONS[i] || { id: '', label: `Dimension ${i + 1}` };
    lines.push(bar);
    lines.push(`${i + 1}. ${meta.label.toUpperCase()}  |  Score: ${dim.score} / 10`);
    lines.push(bar);
    lines.push('');
    lines.push("WHAT'S WRONG:");
    lines.push(dim.issue || '—');
    lines.push('');
    lines.push('FIXED VERSION (copy-paste ready):');
    lines.push(dim.fix || '—');
    if (meta.id === 'returns') {
      lines.push('');
      lines.push(`CRA 2015 COMPLIANCE: ${dim.cra_compliant ? '✓ Compliant' : '✗ Not compliant — see fixed version above'}`);
    }
    lines.push('');
  });

  lines.push(bar);
  lines.push('Returns policy check based on UK Consumer Rights Act 2015.');
  lines.push('Generated by the UK Market Localization Engine.');
  lines.push(bar);

  const blob = new Blob([lines.join('\n')], { type: 'text/plain;charset=utf-8' });
  const url  = URL.createObjectURL(blob);
  const a    = document.createElement('a');
  const safe = (d.brand_name || 'brand').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
  a.href     = url;
  a.download = `uk-report-${safe}.txt`;
